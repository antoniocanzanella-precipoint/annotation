...
public class AnnotationTextPositionAttributeSerializer : IAttributeSerializer
{
    private readonly IByteSerializer _serializer;

    public AnnotationTextPositionAttributeSerializer(IByteSerializer serializer)
    {
    ...
    public int SerializeAttribute(DeckGlLayer<AnnotationShape> layer, LayerHeaderDto header, Span<byte> target)
    {
        ...
        for (var index = 0; index < layer.Data.Count; index++)
        {
            ...
            Coordinate cords = annota.Shape.Coordinates[0];
            var coordsOffset = 0;
            int count = annota.Label.EnumerateRunes().Count();
            for (var i = 0; i < count; i++)
            {
                coordsOffset +=
                    AnnotationAttributeSerializerHelper.SerializeCoordinates(attribute, cords, _serializer,
                        buf.Slice(coordsOffset));
            }
