...
public class CanDeleteAnnotationByValueResolver : IValueResolver<AnnotationShape, AnnotationDto, bool>
{
    private readonly IClaimsPrincipalProvider _claimsPrincipalProvider;
    private readonly IStringLocalizer _stringLocalizer;

    public CanDeleteAnnotationByValueResolver(IClaimsPrincipalProvider claimsPrincipalProvider,
    ...
    public bool Resolve(AnnotationShape source, AnnotationDto destination, bool destMember, ResolutionContext context)
    {
        try
        {
            BusinessValidation.CheckUserDeletePermission(source, _claimsPrincipalProvider, _stringLocalizer);
            return true;
        }
        catch (ApiException)
        {
