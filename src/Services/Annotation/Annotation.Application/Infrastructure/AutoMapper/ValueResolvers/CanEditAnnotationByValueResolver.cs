...
public class CanEditAnnotationByValueResolver : IValueResolver<AnnotationShape, AnnotationDto, bool>
{
    private readonly IClaimsPrincipalProvider _claimsPrincipalProvider;
    private readonly IStringLocalizer _stringLocalizer;

    public CanEditAnnotationByValueResolver(IClaimsPrincipalProvider claimsPrincipalProvider,
    ...
    public bool Resolve(AnnotationShape source, AnnotationDto destination, bool destMember, ResolutionContext context)
    {
        try
        {
            BusinessValidation.CheckUserWritePermission(source, _claimsPrincipalProvider, _stringLocalizer);
            return true;
        }
        catch (ApiException)
        {
