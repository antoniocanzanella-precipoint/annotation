...
namespace PreciPoint.Ims.Services.Annotation.Domain.Tests.Unit
{
    public class U001FixedPointerSerializer
    {
        ...
        public void U001_001CanSerializeInt()
        {
            ...
            int[] values = new[] {0, Int32.MaxValue, Int32.MinValue, 42, 69, -52354722, -123455};
            foreach (var value in values)
            {
                var wrote = serializer.Serialize(value, span);
        ...
        public void U001_002CanSerializeFloat()
        {
            ...
            float[] values = {0, float.MaxValue, float.MinValue, -42, 42, 0.42424f, 0.13371337f, float.Epsilon, float.NaN, float.NegativeInfinity, float.PositiveInfinity};
            foreach (var value in values)
            {
                var wrote = serializer.Serialize(value, span);
        ...
        public void U001_003CanSerializeDouble()
        {
            ...
            double[] values =
            {
                0, double.Epsilon, double.MaxValue, double.MinValue, double.NaN, double.NegativeInfinity,
                double.PositiveInfinity, -42.000000000001, 42.0d, 1337.1337d
            };
            foreach (var value in values)
            {
                var wrote = serializer.Serialize(value, span);
        ...
        public void U001_004CanSerializeLong()
        {
            ...
            long[] values =
            {
                0, -1, 1, long.MaxValue, long.MinValue, -42, 42, 1337
            };
            foreach (var value in values)
            {
                var wrote = serializer.Serialize(value, span);
